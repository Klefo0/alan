...
def get_owner():
  ...
  owner = input(output_message(colors.GREEN, "Before we begin, what should I call you?    "))
...
def get_name():
  ...
  bot = input(output_message(colors.GREEN, "What would you like to call me?    "))
...
def get_language(data):
  ...
  language = input(output_message(colors.WARNING, "[Press Tab] >>> "))
  return language
...
def get_voice(languages_and_voices, language):
  ...
  gnureadline.parse_and_bind('tab: complete')
  voice = input(output_message(colors.WARNING, "[Press Tab] >>> "))
  return voice